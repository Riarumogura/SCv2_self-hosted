...
## 11. ストレージのファイル/フォルダ操作を右クリックメニュー化

### 変更内容

`StorageExplorer.tsx` のファイル一覧で、各行に常時表示されていた「ダウンロード/削除」「名前変更/移動/削除」ボタン(「操作」列)を削除し、任意のファイル/フォルダ行を右クリックすると操作メニューが表示される方式に変更した。

- 新規コンポーネント `components/app/menus/StorageEntryContextMenu.tsx` を追加(既存の `ContextMenu`/`ContextMenuButton`/`ContextMenuDivider` を使用。削除ボタンは `destructive` propで赤色強調)。ファイル行は「ダウンロード」「削除」、フォルダ行は「名前変更」「移動」「削除」を表示する。
- `StorageExplorer.tsx` のテーブルから「操作」列(ヘッダー・常時表示ボタン群・使用していた `FileActions` styled定義)を削除。
- 名前/アイコン部分のセル内に `use:floating={{ contextMenu: () => <StorageEntryContextMenu ... /> }}` を配線した。9章で判明した制約(SolidJSの `use:` ディレクティブはネイティブDOM要素にしか効かない)を踏まえ、styledコンポーネントの `FileTableCell` ではなく、その内側に置いた素の `<div>` に直接配線している。

### 動作確認

Playwrightで以下を確認した:
- 「操作」列のヘッダーおよび常時表示の操作ボタンが一覧から消えていることを確認。
- フォルダ行を右クリック→「名前変更」「移動」「削除」が表示され、名前変更が実際に反映されることを確認。
- ファイル行を右クリック→「ダウンロード」「削除」が表示され、削除が実際に反映される(一覧から消える)ことを確認。
- 左クリックでのフォルダを開く/ファイルをプレビューする挙動(8章で実装)は維持されていることを確認(右クリックのコンテキストメニューとは独立した `onClick` ハンドラのまま)。

## 12. ヘッダーの「更新・新規フォルダ・アップロード」ボタンをテキストからアイコンに変更

### 変更内容

`StorageExplorer.tsx` のヘッダー部にあった「更新」「新規フォルダ」「アップロード」のテキストボタンを、`@revolt/ui` の `IconButton` + `Symbol`(マテリアルシンボル)を使った円形アイコンボタンに変更した。

- 更新 → `refresh` アイコン、新規フォルダ → `create_new_folder` アイコン、アップロード → `upload` アイコン。いずれも `variant="filled"` で塗りつぶし円形のアイコンボタンにし、`use:floating={{ tooltip: {...} }}` でホバー時に元のラベル文言をツールチップ表示する(アイコンのみでも操作内容が分かるようにするため)。
- `IconButton` はコード内に `// @codegen directives props=rest include=floating` というコードジェン用マーカーを持つコンポーネントで、`use:floating` のようなSolidJSディレクティブをカスタムコンポーネント越しに正しく転送できるよう設計されている。これは9章・11章で対応した「styledコンポーネントには`use:`ディレクティブが効かない」という制約への、このコードベースにおける正式な回避策であり、今後同様にカスタムコンポーネントへ`use:floating`を付けたい場合は、素のdivでラップするその場しのぎではなく、まず対象コンポーネントがこのコードジェンマーカーに対応しているか確認するのが望ましい。
- `ActionButtons` のstyled定義から、テキストボタン前提だった `& button` のCSSオーバーライド(背景色・パディング等)を削除し、`IconButton` 自身のスタイリングがそのまま反映されるようにした。

### 動作確認

Playwrightで、3つのボタンが円形アイコンボタンに変わっていること、「更新」ボタンにホバーすると同名のツールチップが表示されること、「新規フォルダ」アイコンボタンをクリックすると実際にフォルダが作成されることを確認した。

## 13. ストレージパネルを閉じる×ボタンの追加

### 変更内容

`TextChannel.tsx` の右サイドバーで `sidebarState().state === "storage"` のときに表示するヘッダーに、ストレージパネルを閉じてチャット表示(デフォルトのメンバー一覧サイドバー)に戻す×ボタンを追加した。

- 同ファイル内でカレンダーパネル(`state === "calendar"`)に既に同じ目的の閉じるボタンが実装されていたため、そのパターンをそのまま再利用した。カレンダー専用だった `CalendarSidebarHeader`(flexで右端にボタンを置くためのstyled定義)を `SidebarHeaderRow` という汎用名にリネームし、カレンダー・ストレージの両方で共用するようにした。
- ボタンは `IconButton`(`size="xs"`, `variant="standard"`)+ `Symbol`(`close`)+ `Tooltip`(「ストレージを閉じる」)で構成し、クリックで `setSidebarState({ state: "default" })` を呼ぶ。これによりサイドバーが通常のメンバー一覧表示に戻る。
- なお、`Show when={sidebarState().state !== "default"}` でバインドされた既存の `CLOSE_SIDEBAR` キーバインド(Escape等)も元から効いており、今回追加したのは見た目上のボタンのみ。

### 動作確認

Playwrightで、ストレージエクスプローラーを開いた状態で×ボタンが表示されること、クリックするとパネルが閉じてチャット(メンバー一覧サイドバー)表示に戻ることを確認した。
