...
ログイン/サインアップ不可・ストレージUI未表示・ストレージ作成エラーの3件を修正済み(詳細は `BUGFIX_LOGIN_AND_STORAGE.md` の1〜5章)。続けて、フォルダ操作・容量表示バー・ストレージ管理UI(7章)、インラインプレビュー・サーバーサイド検索(8章)、「ストレージに保存」のLoading固着バグ・File/Text添付のコンテキストメニュー配線漏れの修正(9章)、i18nカタログの再生成(10章)を実装済み。当初挙げていた未実装6項目はすべて完了した。
...
- メッセージ添付ファイル(画像・動画・音声・汎用ファイル・テキスト)の右クリック→「Save To Storage」によるストレージ保存(File/Text型も含めてコンテキストメニューが開くことを確認済み)
- i18nカタログ再生成済み。新規追加メッセージはハッシュ文字列ではなく英語フォールバックで表示される(日本語訳は未着手)

## 未実装・既知の課題(次にやること)

当初挙げていた6項目・派生バグはすべて対応済み。次に着手するとしたら以下が候補(優先度・要否は要相談):

- 新規追加されたi18nメッセージの日本語(および他言語)翻訳。現状は英語フォールバック表示。
